...
export function RegisterForm() {
  ...
  const resolver = useMemo(() => {
    return valibotResolver(registerSchema, { lang: locale })
  }, [locale])
  ...
  async function onSubmit(values: RegisterI) {
    ...
    try {
      ...
      if (!response.ok) {
        const message = await getErrorMessage(response) ?? t('auth.register.errorDefault')
      ...
      setError('root.server', {
        type: 'network',
        message: t('auth.register.errorDefault'),
      })
    }
  }
    ...
    <section>
      ...
      >
        ...
        <Text className="pt-3 text-center text-sm text-foreground/60">
          {t('auth.register.switch.prompt')}
          {' '}
          <Link href="/login" className="text-sm font-semibold text-accent/90 underline-offset-4 transition hover:text-accent hover:underline">
